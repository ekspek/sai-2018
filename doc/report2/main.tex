...
\begin{figure}[ht]
	\centering
	\begin{tikzpicture}
		\begin{axis}[	domain=0:5,y domain=-45:125,view={-30}{25},
			xlabel=$\si{\volt}$,ylabel=$\degree C$, zlabel=$\si{\pascal}$]
			\addplot3[surf,samples=25]{5.639 + 382.1 * x - 0.05732 * y - 69.73 * x^2 + 0.2646 * x * y + 0.0002634 * y^2 + 11.53 * x^3 - 0.05126 * x^2 * y - 0.0002063 * x * y^2 - 0.8314 * x^4 + 0.004329 * x^3 * y + 4.644 * 0.00001 * x^2 * y^2};
			\addplot3[only marks] table [x index = {1}, z index = {0}, y expr = -45, col sep = comma] {inputdata.csv};
			\addplot3[only marks] table [x index = {2}, z index = {0}, y expr = 25, col sep = comma] {inputdata.csv};
			\addplot3[only marks] table [x index = {3}, z index = {0}, y expr = 125, col sep = comma] {inputdata.csv};
		\end{axis}
	\end{tikzpicture}
...
\begin{figure}[ht]
	...
	\resizebox{\textwidth}{!}{\begin{tikzpicture}
		\node (sp1) at (0,0) [rectangle, draw, align=center,minimum width = 4cm, minimum height = 3cm] {Static \\ Pressure \\ Sensor};
		\node (tx11) at (sp1.north east) [rectangle,draw,anchor = north east] {\texttt{Tx}};
		\path [draw] (tx11.north west) -- (tx11.north west|-sp1.south);
		\node (sp2) at (0,-4) [rectangle, draw, align=center,minimum width = 4cm, minimum height = 3cm] {Static \\ Pressure \\ Sensor};
		\node (tx21) at (sp2.north east) [rectangle,draw,anchor = north east] {\texttt{Tx}};
		\path [draw] (tx21.north west) -- (tx21.north west|-sp2.south);
		\node (sp3) at (0,-8) [rectangle, draw, align=center,minimum width = 4cm, minimum height = 3cm] {Static \\ Pressure \\ Sensor};
		\node (tx31) at (sp3.north east) [rectangle,draw,anchor = north east] {\texttt{Tx}};
		\path [draw] (tx31.north west) -- (tx31.north west|-sp3.south);
		\path (sp1.north east) ++(3,0) node (adc) [anchor = north west, rectangle, draw, align=center, minimum width = 5cm, minimum height = 10cm] {Air Data \\ Computer};
		\node (rxa1) at (adc.north west) [rectangle,draw,anchor = north west] {\texttt{Rx}};
		\path [draw] (rxa1.north east) -- (rxa1.north east|-adc.south);
		...
		\path [draw] (txa1.north west) -- (txa1.north west|-adc.south);
		\path [draw] (tx11) -- (rxa1);
